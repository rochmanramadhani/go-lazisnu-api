...
type (
	Base[T any] interface {
		...
		UpdateByID(ctx context.Context, id uint64, data T) (T, error)
...
func (m *base[T]) Find(ctx context.Context, filterParam abstraction.Filter, search *abstraction.Search) ([]T, *abstraction.PaginationInfo, error) {
	query := m.GetConn(ctx).Debug().Model(m.entity)
	if search != nil {
...
func (m *base[T]) FindByID(ctx context.Context, id uint64) (*T, error) {
	query := m.GetConn(ctx).Debug().Model(m.entity).Where("id", id)
	result := new(T)
...
func (m *base[T]) FindByIDs(ctx context.Context, ids []string, sortBy string) ([]T, error) {
	query := m.GetConn(ctx).Debug().Model(m.entity).Where("id IN ?", ids)
	if sortBy != "" {
		query.Order(sortBy)
	}
...
func (m *base[T]) FindByCode(ctx context.Context, code string) (*T, error) {
	query := m.GetConn(ctx).Debug().Model(m.entity).Where("code", code)
	result := new(T)
	err := query.WithContext(ctx).First(result).Error
	if err != nil {
		return nil, m.MaskError(err)
...
func (m *base[T]) FindByName(ctx context.Context, name string) (*T, error) {
	query := m.GetConn(ctx).Debug().Model(m.entity).Where("name", name)
	result := new(T)
	err := query.WithContext(ctx).First(result).Error
	if err != nil {
		return nil, m.MaskError(err)
...
func (m *base[T]) Create(ctx context.Context, data T) (T, error) {
	err := m.GetConn(ctx).Debug().Model(m.entity).WithContext(ctx).Omit(clause.Associations).Create(&data).Error
...
func (m *base[T]) UpdateByID(ctx context.Context, id uint64, data T) (T, error) {
	err := m.GetConn(ctx).Debug().Model(&data).Select("*").WithContext(ctx).Omit(clause.Associations).Where("id = ?", id).Updates(&data).Error
	return data, m.MaskError(err)
}

func (m *base[T]) DeleteByID(ctx context.Context, id uint64) error {
	err := m.GetConn(ctx).Debug().WithContext(ctx).Where("id = ?", id).Omit(clause.Associations).Delete(m.entity).Error
...
func (m *base[T]) DeleteByIDs(ctx context.Context, ids []string) error {
	...
	err := m.GetConn(ctx).Debug().WithContext(ctx).Where("id IN ?", ids).Omit(clause.Associations).Delete(m.entity).Error
	return m.MaskError(err)
}

func (m *base[T]) Count(ctx context.Context) (int64, error) {
	var count int64
	err := m.GetConn(ctx).Debug().Model(m.entity).WithContext(ctx).Count(&count).Error
	return count, err
}
